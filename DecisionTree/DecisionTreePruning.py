...
def Pruning_node(tree):
    ...
    if node_left.leftChild == None and node_right.leftChild == None:
        ...
        tree.estimate = (node_left.amount * node_left.estimate +
                         node_right.amount * node_right.estimate) / tree.amount
        ...
        tree.leftDepth = max(tree.leftChild.leftDepth,
                             tree.leftChild.rightDepth) + 1
        tree.rightDepth = max(tree.rightChild.rightDepth,
                              tree.rightChild.leftDepth) + 1
        if statues == "OK":
            return "OK"
...
# plot the line and regression tree model in th graph
