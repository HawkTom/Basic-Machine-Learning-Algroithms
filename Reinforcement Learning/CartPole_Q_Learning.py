...
class QCartPoleSolver():
    def __init__(self, buckets=(1, 1, 6, 12,), n_episodes=2000, n_win_ticks=195, min_alpha=0.1, min_epsilon=0.1, gamma=1.0, ada_divisor=25, max_env_steps=None, quiet=False, monitor=False):
    ...
    def run(self):
        ...
        if not self.quiet: print('Did not solve after {} episodes 😞, score: {}'.format(e, mean_score))
        return e
    ...
    def replay(self):
        ...
        G = 0;
        for i in range(1000):
            action = self.choose_action(state, -0.1)
            obs, reward, done, _ = self.env.step(action)
